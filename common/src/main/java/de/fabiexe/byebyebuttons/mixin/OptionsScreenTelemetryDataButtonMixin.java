...
/** This mixin removes the telemetry data button from the options screen */
@Mixin(OptionsScreen.class)
public class OptionsScreenTelemetryDataButtonMixin {
    @WrapOperation(
            method = "init",
            at = @At(
                    value = "INVOKE",
    ...
    public <T extends LayoutElement> T removeButton(GridLayout.RowHelper instance, T widget, Operation<T> original) {
        if (ByeByeButtonsConfig.TELEMETRY_DATA_BUTTON.get()) {
